...
namespace Demo_Tag3
{
	...
	{
		//Enum-Definition
		public enum MathematicOperation { Addition = 1, Subtraktion, Multiplikation, Division }

		public static double Calculate(double number1, double number2, MathematicOperation mo)
		{
			...
			switch (mo)
			{
				...
				case MathematicOperation.Division:
					...
					if (number2 == 0)
					{ 
						throw new DivideByZeroException("Eine Division durch null ist nicht erlaubt.");
					}
					Console.WriteLine($"{number1} / {number2} = {number1 / number2}");
					return number1 / number2;
				default:
		...
		internal static double NumberInput (string text)
		{
			...
			do
			{
				...
				if(incorrectInput)
					throw new IncorrectInputException("Ungültige Eingabe. Bitte erneut versuchen.");
